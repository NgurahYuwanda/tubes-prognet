...
<div class="d-flex  justify-content-end">
  <a href="{{ route('SatuanBarang.create') }}" class="btn btn-sm btn-primary mb-3">Tambah</a>
</div>
  ...
  <script>
    $(document).ready(function () {
     ...
     function loadSatuan(){
       $.ajax({
         type: "GET",
         url: "http://127.0.0.1:8081/api/SatuanBarang",
         dataType: "JSON",
         success: function (response) {
             ...
             if (response.length === 0) {
                 ...
                 $('#table-satuan').html(row);
                 $('.edit-btn').click(function() {
                    var id = $(this).data('id');
                    window.location.href = '/SatuanBarang/' + id + '/edit?data=' + id;
                });
                 $('.delete').click(function() {
                  ...
                  Swal.fire({
                      ...
                      if (result.isConfirmed) {
                        ...
                        $.ajax({
                          type: "DELETE",
                          url: "http://127.0.0.1:8081/api/SatuanBarang/" + id,
                          success: function (response) {
                            loadSatuan()
